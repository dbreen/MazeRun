...
class Maze(object):
    EASY, MEDIUM, HARD, IMPOSSIBRU = range(4)
    WIDTHS = {
        EASY: 48, MEDIUM: 32, HARD: 24, IMPOSSIBRU: 16,
    }
    # Left, Top, Right, Down
    ...
    def draw_marker(self, surface, point, color, radius):
        center = ((point[0] + 1) * self.wallwidth - radius / 2 - constants.WALL_THICKNESS / 2,
                  (point[1] + 1) * self.wallwidth - radius / 2 - constants.WALL_THICKNESS / 2)
        return pygame.draw.circle(surface, color, center, radius)

    def draw_player(self, surface):
        #points = [self.grid_to_screen(point) for point in self.player.points]
        points = self.player.points
        pygame.draw.lines(surface, constants.PLAYER_LINE_COLOR, False, points, constants.PLAYER_LINE_THICKNESS)

    def grid_to_screen(self, pos):
        """Convert grid coordinates to screen coordinates"""
        ww = self.wallwidth
        return ((pos[0] + 1) * ww - ww / 2 + constants.WALL_THICKNESS / 2,
                (pos[1] + 1) * ww - ww / 2 + constants.WALL_THICKNESS / 2)
